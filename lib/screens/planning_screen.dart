...
class _PlanningScreenState extends State<PlanningScreen> with SingleTickerProviderStateMixin {
  ...
  bool _isVistaSettimanale = false; // Controllo vista Giornaliera vs Settimanale
  ...
  Future<void> _completaPomodoro() async {
    _pomodoroTimer?.cancel();
    if (_selectedTaskForPomodoro != null) {
      await Provider.of<PlannerProvider>(context, listen: false).savePomodoroSession(
  ...
  Color _colorePriorita(String priorita) {
    switch (priorita.toLowerCase()) {
      ...
      case 'media': return Colors.orangeAccent;
      case 'bassa': return Colors.green;
      default: return Colors.grey;
    }
  }

  // Widget riutilizzabile per mostrare il badge della priorità (solo per i Task)
  Widget _buildPriorityBadge(String priorita) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(color: colore, fontSize: 9, fontWeight: FontWeight.bold),
      ),
    );
  }

  bool _isSameWeek(DateTime date, DateTime reference) {
    final diff = date.difference(reference).inDays;
    if (diff.abs() >= 7) return false;
    final refInizioSettimana = reference.subtract(Duration(days: reference.weekday - 1));
    final refFineSettimana = reference.add(Duration(days: 7 - reference.weekday));
    
    return (date.isAfter(refInizioSettimana.subtract(const Duration(seconds: 1))) && 
            date.isBefore(refFineSettimana.add(const Duration(seconds: 1))));
  }
  ...
  Widget build(BuildContext context) {
    ...
    // ================= DATI TAB 1 (OGGI) =================
    final sessioniOggi = provider.studySessions.where((s) => DateUtils.isSameDay(s.data, oggi)).toList();
    final taskOggi = provider.tasks.where((t) => t.scadenza != null && DateUtils.isSameDay(t.scadenza, oggi)).toList();
    
    // Ordiniamo solo i Task per priorità. Le sessioni vengono ordinate per orario/cronologia (data)
    taskOggi.sort((a, b) => _pesoPriorita(b.priorita).compareTo(_pesoPriorita(a.priorita)));
    ...
    // Rimosso l'ordinamento per priorità anche da qui (le ordiniamo cronologicamente)
    sessioniPianificatore.sort((a, b) => a.data.compareTo(b.data));
    
    if (_filtroCorso != null) {
      sessioniPianificatore = sessioniPianificatore.where((s) => s.courseId == _filtroCorso!.id).toList();
    }
    if (_filtroTipoAttivita != 'Tutti') {
      sessioniPianificatore = sessioniPianificatore.where((s) => s.tipo.toLowerCase() == _filtroTipoAttivita.toLowerCase()).toList();
    }

    return Scaffold(
      appBar: AppBar(
        title: const Text('Planning & Focus', style: TextStyle(fontWeight: FontWeight.bold)),
        bottom: TabBar(
      ...
      floatingActionButton: _tabController.index == 1
          ? FloatingActionButton.extended(
              ...
              label: const Text('Pianifica', style: TextStyle(color: Colors.white)),
            )
          : null,
    );
  }
    ...
    return ListView(
      ...
      children: [
        if (taskInCorso.isNotEmpty || sessioniInCorso.isNotEmpty) ...[
          Text('Da Completare Oggi', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Colors.blueGrey)),
        ...
        if (taskCompletati.isNotEmpty || sessioniCompletate.isNotEmpty) ...[
          Row(
            children: [
              ...
              Text('Attività Completate', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Colors.green)),
            ],
          ),
          const SizedBox(height: 8),
          ...taskCompletati.map((t) => Opacity(opacity: 0.6, child: _buildTaskCard(t, provider))),
          ...sessioniCompletate.map((s) => Opacity(opacity: 0.6, child: _buildSessionCard(s, provider))),
        ],
      ],
    );
  }
  ...
  Widget _buildTaskCard(Task t, PlannerProvider provider) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(t.titolo, style: TextStyle(decoration: t.completata ? TextDecoration.lineThrough : null)),
        subtitle: Text(provider.getCourseById(t.courseId ?? '')?.nome ?? 'Generico'),
        trailing: _buildPriorityBadge(t.priorita),
      ),
    );
  }

  Widget _buildSessionCard(StudySession s, PlannerProvider provider, {bool mostraData = false}) {
    String dataString = mostraData ? '${DateFormat('dd/MM').format(s.data)} • ' : '';
    return Card(
      child: ListTile(
        leading: Icon(
          s.tipo.toLowerCase() == 'consegna' ? Icons.assignment_turned_in : Icons.menu_book, 
          color: s.tipo.toLowerCase() == 'consegna' ? Colors.redAccent : AppColors.planning
        ),
        title: Row(
          children: [
        ...
        trailing: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: () => _apriDialogPianificazione(context, sessioneEsistente: s),
            ),
            Checkbox(
              value: s.completata,
              onChanged: (v) => provider.updateStudySession(s.copyWith(completata: v!)),
            ),
          ],
        ),
      ),
  ...
  // --- TAB 2: PIANIFICATORE ---
  Widget _buildTabPianificatore(List<StudySession> sessioni, PlannerProvider provider) {
    final sessioniInCorso = sessioni.where((s) => !s.completata).toList();
    final sessioniCompletate = sessioni.where((s) => s.completata).toList();
    ...
    return Column(
      children: [
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                onPressed: () async {
                  ...
                  if (picked != null) setState(() => _giornoPianificatore = picked);
                },
        ...
        ListTile(
          title: Text(
            _isVistaSettimanale
                ? 'Settimana del ${DateFormat('dd MMMM').format(_giornoPianificatore.subtract(Duration(days: _giornoPianificatore.weekday - 1)))}'
                : DateFormat('EEEE dd MMMM').format(_giornoPianificatore),
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
          ),
          leading: Icon(_isVistaSettimanale ? Icons.date_range_outlined : Icons.calendar_today, color: AppColors.planning),
        ),
        ...
        Padding(
          ...
          child: ExpansionTile(
            ...
            title: const Text('Filtra attività del periodo', style: TextStyle(fontSize: 14)),
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<Course?>(
                        decoration: const InputDecoration(labelText: 'Corso', contentPadding: EdgeInsets.symmetric(horizontal: 10)),
                        value: _filtroCorso,
                        hint: const Text('Tutti'),
                        items: [
                          const DropdownMenuItem<Course?>(value: null, child: Text('Tutti i Corsi')),
                          ...provider.courses.map((c) => DropdownMenuItem(value: c, child: Text(c.nome)))
                        ],
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        decoration: const InputDecoration(labelText: 'Tipo', contentPadding: EdgeInsets.symmetric(horizontal: 10)),
                        value: _filtroTipoAttivita,
                        items: ['Tutti', 'Studio', 'Ripasso', 'Esercitazione', 'Progetto', 'Consegna']
                            .map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                        onChanged: (v) => setState(() => _filtroTipoAttivita = v!),
                      ),
                    ),
                  ],
                ),
        ...
        Expanded(
          child: sessioni.isEmpty
              ? const Center(child: Text('Nessun impegno corrisponde ai criteri.', style: TextStyle(color: Colors.grey)))
              : ListView(
                  padding: const EdgeInsets.symmetric(vertical: 8),
                  children: [
                    if (sessioniInCorso.isNotEmpty) ...[
                      ...sessioniInCorso.map((s) => _buildPianificatoreCard(s, provider)),
                    ],
                      ...
                      const Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text('Sessioni Completate', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 13)),
                          ],
                        ),
                      ),
                      ...sessioniCompletate.map((s) => Opacity(opacity: 0.5, child: _buildPianificatoreCard(s, provider))),
                    ],
                  ],
                ),
        )
  ...
  Widget _buildPianificatoreCard(StudySession s, PlannerProvider provider) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${_isVistaSettimanale ? "${DateFormat('EE dd/MM').format(s.data)} • " : ""}${provider.getCourseById(s.courseId ?? '')?.nome ?? "Generico"} • Tipo: ${s.tipo}',
        ),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            ...
            IconButton(
              ...
              onPressed: () => provider.deleteStudySession(s.id),
            ),
          ],
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          DropdownButtonFormField<Task>(
            ...
            items: pendingTasks.map((t) => DropdownMenuItem(value: t, child: Text(t.titolo))).toList(),
            onChanged: _isTimerRunning ? null : (task) => setState(() => _selectedTaskForPomodoro = task),
          ),
          const Spacer(),
          Stack(
          ...
          Row(
            ...
            children: [
              FloatingActionButton.extended(
                heroTag: 'btnStart',
                onPressed: _isTimerRunning ? _pausePomodoro : _startPomodoro,
                ...
                label: Text(_isTimerRunning ? 'Pausa' : 'Inizia Focus', style: const TextStyle(color: Colors.white)),
              ),
  ...
  // --- DIALOG UNIFICATO ---
  void _apriDialogPianificazione(BuildContext context, {StudySession? sessioneEsistente}) {
    final provider = Provider.of<PlannerProvider>(context, listen: false);
    
    final isModifica = sessioneEsistente != null;
    final titController = TextEditingController(text: isModifica ? sessioneEsistente.titolo : '');
    
    Course? corsoScelto;
    if (isModifica) {
      try {
        corsoScelto = provider.courses.firstWhere((c) => c.id == sessioneEsistente.courseId);
      } catch (_) {
        corsoScelto = null;
      }
    }
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Cosa farai? (Titolo) *'),
                ),
                const SizedBox(height: 8),
                DropdownButtonFormField<Course>(
                  decoration: const InputDecoration(labelText: 'Corso *'),
                  value: corsoScelto,
                  items: provider.courses.map((c) => DropdownMenuItem(value: c, child: Text(c.nome))).toList(),
                ...
                DropdownButtonFormField<String>(
                  ...
                  value: tipoStudio,
                  items: ['Studio', 'Ripasso', 'Esercitazione', 'Progetto', 'Consegna']
                      .map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (titController.text.trim().isEmpty || corsoScelto == null) return;
